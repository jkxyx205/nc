...
$.fn.jqGrid = function( pin ) {
	...
	return this.each( function() {
		...
		var p = $.extend(true,{
			...
			ExpandColumn: null,
			tree_root_level : 0,
			prmNames: {page:"page",rows:"rows", sort: "sidx",order: "sord", search:"_search",reloadAll:'reloadAll',queryName:'queryName', nd:"nd", id:"id",oper:"oper",editoper:"edit",addoper:"add",deloper:"del", subgridid:"id", npage: null, totalrows:"totalrows"},
			forceFit : false,
			gridstate : "visible",
			cellEdit: false,
			cellsubmit: "remote",
		...
		populate = function (npage) {
			if(!ts.grid.hDiv.loading) {
				...
				if(pN.reloadAll !== null) {prm[pN.reloadAll] = ts.p.reloadAll;} 
...
$.jgrid.extend({
	...
	navGrid : function (elem, o, pEdit,pAdd,pDel,pSearch, pView,pExp) {
		o = $.extend({
			exp:true, //add by rick
			expicon:"ui-icon-link",
			edit: false,
		...
		return this.each(function() {
			...
			for(i = 0; i<clone; i++) {
				...
				if (o.exp) { //add by rick 
					tbd = $("<td class='ui-pg-button ui-corner-all'></td>");
					pExp = pExp || {};
					$(tbd).append("<div class='ui-pg-div'><span class='ui-icon "+o.expicon+"'></span>"+o.viewtext+"</div>");
					$("tr",navtbl).append(tbd);
					$(tbd,navtbl)
					.attr({"title":o.exptitle || "",id: pExp.id || "exp_"+elemids})
					.click(function(){
						if($t.p.queryForm != undefined && !$($t.p.queryForm).validationEngine("validate"))
							return;
						
						function getParam() {
							var param = {};
							if($t.p.queryForm != undefined) {
								param = $($t.p.queryForm).form2json({allowEmptyMultiVal:true});
							}
							
							if($t.p.data != undefined) {
								$.extend(param,$t.p.data);
							}
						...
						var json = {
								queryName:$t.p.queryName,
								reloadAll:$t.p.reloadAll,
								fileName:$t.p.fileName,
								sidx:$t.p.sortname,
								sord:$t.p.sortorder,
								colNames:$t.p.colNames,
